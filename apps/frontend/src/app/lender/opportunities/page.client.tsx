        ...
        {loading ? (
          ...
          <div className="bg-white border-[1.5px] border-stone-200 rounded-[8px] p-8">
            ...
            <p className="text-sm text-stone-500 mt-1">
              There are currently no eligible borrower applications matching your lending priorities.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {opportunities.filter((o) => !rejectedIds.has(o.applicationId)).map((opportunity) => {
              ...
              return (
                <article key={opportunity.applicationId} className="bg-white border-[1.5px] border-stone-200 rounded-[8px] p-5">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    {band && <Badge variant={band.tone} size="sm" dot>{band.label}</Badge>}
                  </div>

                  {opportunity.purposeDescription && (
                    <p className="text-sm text-stone-600 mt-3">{opportunity.purposeDescription}</p>
                  )}

                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
                    <div>
                      <p className="text-[11px] uppercase tracking-wide text-stone-400">Requested</p>
                      <p className="text-sm font-semibold text-navy mt-0.5">{formatTaka(opportunity.requestedAmount)}</p>
                    </div>
                    <div>
                      <p className="text-[11px] uppercase tracking-wide text-stone-400">Remaining</p>
                      <p className="text-sm font-semibold text-navy mt-0.5">{formatTaka(remaining)}</p>
                    </div>
                    <div>
                      <p className="text-[11px] uppercase tracking-wide text-stone-400">Interest</p>
                      <p className="text-sm font-semibold text-navy mt-0.5">{opportunity.interestRate == null ? "—" : formatPercent(opportunity.interestRate)}</p>
                    </div>
                    <div>
                      <p className="text-[11px] uppercase tracking-wide text-stone-400">Tenure</p>
                      <p className="text-sm font-semibold text-navy mt-0.5">{opportunity.durationMonths == null ? "—" : `${opportunity.durationMonths} mo`}</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 rounded-[6px] bg-stone-50 border border-stone-200 p-4">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-[11px] uppercase tracking-wide text-stone-400">Borrower trust score</p>
                        <p className="text-xl font-semibold text-navy mt-0.5">
                          {opportunity.trustScore == null ? "Not available" : `${opportunity.trustScore}/100`}
                        </p>
                      </div>
                      <button type="button" className="text-xs font-medium text-teal hover:underline" onClick={() => toggle(opportunity.applicationId)}>
                    ...
                    {isExpanded && (
                      <div className="mt-4 border-t border-stone-200 pt-3 flex flex-col gap-2.5">
                        {opportunity.trustFactors.length === 0 ? (
                          <p className="text-xs text-stone-500">No factor-level trust data is available.</p>
                        ) : (
                            ...
                            <div key={`${opportunity.applicationId}-${factor.name}`}>
                              <div className="flex items-center justify-between gap-3 text-xs">
                                ...
                                <span className="tabular-nums text-stone-500">
                                  {factor.score}{factor.weight == null ? "" : ` · ${Math.round(factor.weight * 100)}% weight`}
                                </span>
                              </div>
                              {factor.description && <p className="text-[11px] text-stone-500 mt-0.5">{factor.description}</p>}
                            </div>
                          ))
                        )}
                      </div>
                  ...
                  <div className="mt-4 flex flex-col sm:flex-row gap-2">
                    <input
                      ...
                      onChange={(event) => setFundAmounts((current) => ({ ...current, [opportunity.applicationId]: event.target.value }))}
