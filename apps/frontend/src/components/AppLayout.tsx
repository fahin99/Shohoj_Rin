...
const borrowerNav: SidebarItem[] = [
  ...
  },
  {
    ...
    icon: <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="7" cy="7" r="5" stroke="currentColor" strokeWidth="1.5"/><path d="M11 11L14 14" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>,
  },
  {
    label: 'My Loans', page: 'active-loan',
    icon: <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><rect x="2" y="4" width="12" height="9" rx="1" stroke="currentColor" strokeWidth="1.5"/><path d="M5 4V3a3 3 0 016 0v1" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>,
  },
  {
    label: 'Applications', page: 'application-status',
    icon: <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M3 2h10a1 1 0 011 1v10a1 1 0 01-1 1H3a1 1 0 01-1-1V3a1 1 0 011-1z" stroke="currentColor" strokeWidth="1.5"/><path d="M5 6h6M5 9h4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>,
    badge: 2,
  },
  {
    label: 'Repayments', page: 'repayment',
...
function NavItem({ item, active, onClick }: { item: SidebarItem; active: boolean; onClick: () => void }) {
  return (
    ...
    >
      ...
      {item.badge && (
        <Badge variant={active ? 'neutral' : 'teal'} size="sm">{item.badge}</Badge>
      )}
    </button>
  );
}
          ...
          <div className="relative">
            ...
            >
              <svg width="15" height="15" viewBox="0 0 16 16" fill="none">
                <path d="M8 14a1.5 1.5 0 003 0H5a1.5 1.5 0 003 0zM14 11c-1-1-2-2-2-5a4 4 0 10-8 0c0 3-1 4-2 5h12z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" />
              </svg>
            ...
            {notifOpen && (
              <div className="absolute right-0 top-full mt-2 w-72 bg-white border-[1.5px] border-navy shadow-nb rounded-[8px] overflow-hidden z-10">
                <div className="px-4 py-3 border-b border-stone-200 flex items-center justify-between">
                  ...
                  <span className="text-xs text-teal cursor-pointer hover:underline">Mark all read</span>
                </div>
                <div className="max-h-72 overflow-y-auto flex flex-col gap-px p-2">
                  {[
                    { title: 'Repayment due in 3 days', msg: 'Your EMI of ৳4,500 is due on Dec 15.', time: '2h ago', unread: true },
                    { title: 'Application update', msg: 'Your loan application SR-2025-4812 is under review.', time: '1d ago', unread: true },
                    { title: 'Payment confirmed', msg: 'Your repayment of ৳4,500 was received successfully.', time: '3d ago', unread: false },
                  ].map((n, i) => (
                    <div key={i} className={`px-3 py-2.5 rounded-[4px] ${n.unread ? 'bg-teal-light' : 'hover:bg-stone-50'}`}>
                      <div className="flex min-w-0 items-start gap-2">
                        {n.unread && <span className="w-1.5 h-1.5 rounded-full bg-teal mt-1.5 shrink-0" />}
