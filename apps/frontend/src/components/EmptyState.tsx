...
export function EmptyState({
  ...
  return (
    ...
    >
      ...
      <h3 className={`font-semibold text-navy mb-1 ${size === 'sm' ? 'text-sm' : 'text-base'}`}>{title}</h3>
      {description && (
        <p className={`text-stone-500 leading-relaxed max-w-xs ${size === 'sm' ? 'text-xs' : 'text-sm'}`}>
          {description}
        </p>
      )}
      {(action || secondaryAction) && (
        <div className="flex items-center gap-2 mt-5">
          ...
          {secondaryAction && (
            <Button variant="secondary" size={size === 'sm' ? 'sm' : 'md'} onClick={secondaryAction.onClick}>
...
const EmptyIcons = {
  ...
  transactions: (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      <path d="M7 16l-4-4 4-4M17 8l4 4-4 4M14 4l-4 16" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  ),
  notifications: (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
