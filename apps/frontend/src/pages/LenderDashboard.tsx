...
const fundedLoans: FundedLoan[] = [
  { id: 'SR-2026-004812', borrowerAlias: 'Borrower R.A.', product: 'Student Tuition Support Loan', amount: 200000, rate: 8.5, tenure: 48, repaidPct: 29, status: 'active' },
  { id: 'SR-2026-004531', borrowerAlias: 'Borrower M.K.', product: 'Small Business Working Capital Facility', amount: 450000, rate: 12, tenure: 36, repaidPct: 62, status: 'active' },
  { id: 'SR-2026-004229', borrowerAlias: 'Borrower S.J.', product: 'Emergency Medical Assistance', amount: 60000, rate: 10.5, tenure: 24, repaidPct: 100, status: 'closed' },
  { id: 'SR-2026-004108', borrowerAlias: 'Borrower T.H.', product: 'Personal Flexible Loan', amount: 120000, rate: 11.25, tenure: 30, repaidPct: 8, status: 'overdue' },
  { id: 'SR-2026-003987', borrowerAlias: 'Borrower N.A.', product: 'Rural Entrepreneur Growth Loan', amount: 300000, rate: 10, tenure: 24, repaidPct: 45, status: 'active' },
];
...
const opportunities: Opportunity[] = [
  { id: 'op-1', product: 'Skills & Professional Development Loan', provider: 'Shohoj Learn Finance', amount: 80000, rate: 9, tenure: 18, risk: 'Low' },
  { id: 'op-2', product: 'Small Business Working Capital Facility', provider: 'Dhaka Trade Credit', amount: 350000, rate: 12, tenure: 36, risk: 'Medium' },
  { id: 'op-3', product: 'Emergency Medical Assistance', provider: 'Shohoj Care Finance', amount: 45000, rate: 10.5, tenure: 24, risk: 'Medium' },
];
...
export default function LenderDashboard({ onNavigate }: Props) {
  ...
  const columns: Column<FundedLoan>[] = [
    ...
    { key: 'rate', header: 'Rate', numeric: true, hideBelow: 'sm', render: (r) => formatPercent(r.rate) },
    { key: 'tenure', header: 'Tenure', numeric: true, hideBelow: 'lg', render: (r) => `${r.tenure} mo` },
    ...
    <AppLayout onNavigate={onNavigate} currentPage="lender-dashboard" userType="lender" userName={userName}>
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
          <StatCard label="Total deployed" value={formatTaka(2340000)} hint="Across 5 active loans" />
          ...
          <StatCard label="At-risk exposure" value={formatTaka(120000)} tone="critical" hint="1 overdue loan" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-6">
          ...
          <div className="lg:col-span-2 bg-white border-[1.5px] border-stone-200 rounded-[8px] p-5 min-w-0">
            <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-4 sm:flex sm:justify-between items-start mb-5">
              <div className="min-w-0">
                ...
                <p className="text-xs text-stone-500 mt-0.5">Monthly disbursed amount, last 6 months</p>
              </div>
            </div>
          ...
          <div className="bg-white border-[1.5px] border-stone-200 rounded-[8px] p-5 min-w-0">
            ...
            <div className="flex flex-col gap-4">
              {riskBreakdown.map((r) => (
                <ProgressBar key={r.label} label={r.label} value={r.value} showValue color={r.color} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border-[1.5px] border-stone-200 rounded-[8px] mb-6">
          ...
          <DataTable caption="Funded loans" columns={columns} rows={fundedLoans} rowKey={(r) => r.id} />
        </div>
        ...
        <div className="bg-white border-[1.5px] border-stone-200 rounded-[8px] p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {opportunities.map((op) => {
              ...
              return (
                <div key={op.id} className="border-[1.5px] border-stone-200 rounded-[6px] p-4 flex flex-col gap-3 min-w-0">
                  <div className="min-w-0">
                  ...
                  <div className="flex items-center justify-between gap-2 mt-auto pt-2 border-t border-stone-100">
                    <Badge variant={op.risk === 'Low' ? 'success' : op.risk === 'Medium' ? 'warning' : 'error'} size="sm" dot>
                      {op.risk} risk
                    </Badge>
                    <Button
