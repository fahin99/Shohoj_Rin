...
import { Skeleton, SkeletonText, SkeletonCard, SkeletonRow, SkeletonDashboard } from '../components/Skeleton';
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6 py-6">
        ...
        <Section title="Loading states" caption="Skeletons communicate structure while data loads. Never use spinners alone for content-heavy areas.">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="bg-white border-[1.5px] border-stone-200 rounded-[8px] p-5">
              <p className="text-xs font-medium text-stone-500 mb-3 uppercase tracking-wide">Skeleton card</p>
              <SkeletonCard />
            </div>
            <div className="bg-white border-[1.5px] border-stone-200 rounded-[8px] p-5">
              <p className="text-xs font-medium text-stone-500 mb-3 uppercase tracking-wide">Skeleton rows</p>
              <div className="flex flex-col">
                {Array.from({ length: 3 }).map((_, i) => <SkeletonRow key={i} />)}
              </div>
            </div>
            <div className="bg-white border-[1.5px] border-stone-200 rounded-[8px] p-5">
              <p className="text-xs font-medium text-stone-500 mb-3 uppercase tracking-wide">Skeleton text + block</p>
            ...
            <div className="bg-white border-[1.5px] border-stone-200 rounded-[8px] p-5">
              <p className="text-xs font-medium text-stone-500 mb-3 uppercase tracking-wide">Skeleton dashboard</p>
              <SkeletonDashboard />
            </div>
          </div>
        </Section>
        ...
        <Section title="Error states" caption="Alerts communicate status without relying on colour alone — each carries an icon and label.">
          <div className="flex flex-col gap-3 mb-5">
            ...
            <Alert variant="success" title="Success">Your repayment of ৳4,500 was received successfully.</Alert>
            <Alert variant="warning" title="Action needed">Your application is missing a guarantor NID copy.</Alert>
            <Alert variant="error" title="Something went wrong" dismissible>We couldn't process your payment. Please try again.</Alert>
          </div>
          <div className="bg-white border-[1.5px] border-coral/40 rounded-[8px] p-6 flex flex-col items-center text-center">
            ...
            <p className="text-sm text-stone-500 max-w-xs mb-4">
              There was a problem connecting to the server. Retried {retryCount} time{retryCount === 1 ? '' : 's'}.
            </p>
        ...
        <Section title="Other page states" caption="Full-page states for connectivity, missing routes, and access control.">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div className="bg-white border-[1.5px] border-stone-200 rounded-[8px] p-5 flex flex-col items-center text-center">
              <div className="w-10 h-10 rounded-full bg-stone-100 border border-stone-200 flex items-center justify-center text-stone-400 mb-3">⌁</div>
              <h3 className="font-semibold text-navy text-sm mb-1">You're offline</h3>
              <p className="text-xs text-stone-500 mb-3">Check your internet connection and try again.</p>
            ...
            <div className="bg-white border-[1.5px] border-stone-200 rounded-[8px] p-5 flex flex-col items-center text-center">
              <div className="w-10 h-10 rounded-full bg-yellow-light border border-yellow/40 flex items-center justify-center text-stone-700 mb-3">⛔</div>
              <h3 className="font-semibold text-navy text-sm mb-1">Permission denied</h3>
              <p className="text-xs text-stone-500 mb-3">You don't have access to view this page.</p>
              <Badge variant="warning" size="sm" dot>Restricted</Badge>
            </div>
          </div>
        </Section>
        {}
        ...
        <Section title="Inputs" caption="Default, focus (try clicking), error, and disabled states.">
          <div className="bg-white border-[1.5px] border-stone-200 rounded-[8px] p-5 grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <TextInput label="Input with error" placeholder="Enter amount" error="This field is required" />
        ...
        <Section title="Badges" caption="Status is always shown with a label and dot indicator, never colour alone.">
          <div className="bg-white border-[1.5px] border-stone-200 rounded-[8px] p-5 flex flex-col gap-5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge variant="teal" dot>Teal</Badge>
              <Badge variant="sky" dot>Sky</Badge>
            </div>
            <div className="min-w-0">
              <p className="text-xs font-medium text-stone-500 mb-2 uppercase tracking-wide">Loan status badges</p>
              <div className="flex flex-wrap items-center gap-2">
                {loanStatuses.map((s) => <LoanStatusBadge key={s} status={s} />)}
              </div>
            </div>
            <div className="min-w-0">
              <p className="text-xs font-medium text-stone-500 mb-2 uppercase tracking-wide">Application status badges</p>
              <div className="flex flex-wrap items-center gap-2">
                {appStatuses.map((s) => <AppStatusBadge key={s} status={s} />)}
              </div>
            </div>
          </div>
        </Section>
