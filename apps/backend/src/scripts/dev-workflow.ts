...
async function api(path: string, opts: CallOpts = {}): Promise<ApiResult> {
  ...
  if (opts.token) headers.Cookie = `shohojrin_session=${encodeURIComponent(opts.token)}`;
...
async function call(path: string, opts: CallOpts, expected: number[], label: string): Promise<any> {
...
function extractSessionId(headers: Headers): string {
  const setCookie = (headers as any).getSetCookie?.() ?? [];
  for (const raw of setCookie) {
    const match = raw.match(/^shohojrin_session=([^;]+)/);
...
async function registerUser(username: string, role: "borrower" | "lender") {
  ...
  return {
    ...
    email,
    token: extractSessionId(result.headers),
  };
}
...
async function loginAdmin() {
  ...
  if (user.role !== "admin") {
    throw new Error(`Configured admin account ${ADMIN_EMAIL} has role '${user.role}', not 'admin'`);
  }
...
interface WorkflowContext {
  ...
  applications: Record<
    string,
    { applicationId: string; borrowerKey: string; purpose: string; amount: number }
  >;
  loanId: string;
  ...
  try {
    ...
    await runStage("Register lenders + complete investor profiles", async () => {
      const defs: LenderDef[] = [
        {
          key: "la1",
          username: `alpha_investments_${RUN_TAG}`,
          name: "Alpha Investments",
          categories: ["education", "development"],
        },
        {
          key: "la2",
          username: `omega_capital_${RUN_TAG}`,
          name: "Omega Capital",
          categories: ["education", "development"],
        },
        {
          key: "lb",
          username: `beta_microfinance_${RUN_TAG}`,
          name: "Beta Microfinance",
      ...
      for (const def of defs) {
        ...
        console.log(
          `  ✅ Lender ${def.username} (${reg.userId}) — investor profile completed, prefers ${def.categories.join(", ")}`,
        );
      }
    });
    ...
    await runStage("Register borrowers + complete profiles + trust scores", async () => {
      const defs: BorrowerDef[] = [
        {
          key: "ba",
          username: `anika_rahman_${RUN_TAG}`,
          name: "Anika Rahman",
          purpose: "education",
          amount: 180000,
        },
        {
          key: "bb",
          username: `bakhtiar_hossain_${RUN_TAG}`,
      ...
      for (const def of defs) {
        ...
        check(
          `trust_${def.key}_db`,
          ok,
          `trust score persisted & current (${tsRows.length} row(s))`,
        );
        console.log(
          `  ✅ Borrower ${def.username} (${reg.userId}) — profile completed, trust score recalculated`,
        );
      }
    });
    ...
    await runStage("Submit loan applications via POST /applications", async () => {
      ...
      if (!eduProduct || !bizProduct)
        throw new Error("Missing loan product for education/business purpose");
      ...
      ctx.applications.appA = {
        applicationId: appA.applicationId,
        borrowerKey: "ba",
        purpose: "education",
        amount: 180000,
      };
      ctx.applications.appB1 = {
        applicationId: appB1.applicationId,
        borrowerKey: "bb",
        purpose: "business",
        amount: 200000,
      };
      const dbAppA = await dbRows(
        `SELECT status FROM loan_applications WHERE application_id = $1`,
        [appA.applicationId],
      );
      check(
        "application_in_db_submitted",
        dbAppA[0]?.status === "submitted",
        `application persisted in PostgreSQL (status=${dbAppA[0]?.status})`,
      );
      const myApps = await getApplications(ba.token);
      check(
        "borrower_sees_own_application",
        myApps.applications?.some((a: any) => a.applicationId === appA.applicationId),
        "borrowerA lists its application via GET /applications",
      );
    });
    ...
    await runStage("Lender matching & opportunities visibility", async () => {
      ...
      check(
        "lender_la1_sees_appA",
        oppsLa1.some((o: any) => o.applicationId === appAId),
        "appA visible in GET /investor/opportunities",
      );
      check(
        "lender_la1_does_not_see_appB1",
        !oppsLa1.some((o: any) => o.applicationId === appB1Id),
        "business app not leaked to education lender",
      );
      const oppLb = await getOpportunities(lb.token);
      const oppsLb: any[] = Array.isArray(oppLb) ? oppLb : Object.values(oppLb ?? {});
      check(
        "lender_lb_sees_appB1",
        oppsLb.some((o: any) => o.applicationId === appB1Id),
        "appB1 visible in GET /investor/opportunities",
      );
      check(
        "lender_lb_does_not_see_appA",
        !oppsLb.some((o: any) => o.applicationId === appAId),
        "education app not leaked to business lender",
      );
      ...
      check(
        "lender_application_matches_rows",
        Number(matchRows[0]?.n) >= 3,
        `lender_application_matches rows in PostgreSQL: ${matchRows[0]?.n}`,
      );
    });
    await runStage("Funding commitments & negative boundary rules", async () => {
      ...
      const prematureLoan = await api("/loans", {
        method: "POST",
        token: la1.token,
        body: { applicationId: appAId },
      });
      check(
        "rule_a_lender_not_fully_funded",
        prematureLoan.status === 400,
        `lender loan creation on partial funding → ${prematureLoan.status} (${prematureLoan.payload?.error?.message ?? ""})`,
      );
      const borrowerPremature = await api("/loans", {
        method: "POST",
        token: ba.token,
        body: { applicationId: appAId },
      });
      check(
        "rule_a_borrower_not_fully_funded",
        borrowerPremature.status === 400 || borrowerPremature.status === 403,
        `borrower loan creation on partial funding → ${borrowerPremature.status}`,
      );

      // Rule D: funding amount cannot exceed the remaining requested amount.
      const overFund = await api(`/investor/fund/${appAId}`, {
        method: "POST",
        token: la2.token,
        body: { amount: 160000 },
      });
      check(
        "rule_d_overfund_rejected",
        overFund.status === 400,
        `over-funding attempt (160000 vs remaining 150000) → ${overFund.status} (${overFund.payload?.error?.message ?? ""})`,
      );
      ...
      check(
        "funding_commitments_in_db",
        Number(mentsRows[0]?.total) === 180000 && Number(mentsRows[0]?.n) === 2,
        `funding_commitments in PostgreSQL: n=${mentsRows[0]?.n}, sum=${mentsRows[0]?.total} (fully funded)`,
      );

      // Rule E: duplicate funding rejected.
      const dup1 = await api(`/investor/fund/${appAId}`, {
        method: "POST",
        token: la1.token,
        body: { amount: 1000 },
      });
      check(
        "rule_e_dup_funding_la1",
        dup1.status === 409,
        `duplicate funding by lenderA1 → ${dup1.status}`,
      );
      const dup2 = await api(`/investor/fund/${appAId}`, {
        method: "POST",
        token: la2.token,
        body: { amount: 1000 },
      });
      check(
        "rule_e_dup_funding_la2",
        dup2.status === 409,
        `duplicate funding by lenderA2 → ${dup2.status}`,
      );
      ...
      const prematureB = await api("/loans", {
        method: "POST",
        token: lb.token,
        body: { applicationId: ctx.applications.appB1.applicationId },
      });
      check(
        "rule_a_partial_second_app",
        prematureB.status === 400,
        `lenderB loan creation on partially funded appB1 → ${prematureB.status}`,
      );
      ...
      check(
        "app_b1_rejected",
        appB1Detail.status === "rejected",
        "appB1 rejected through real admin API",
      );
      const loanFromRejected = await api("/loans", {
        method: "POST",
        token: lb.token,
        body: { applicationId: ctx.applications.appB1.applicationId },
      });
      check(
        "rule_f_rejected_app_blocked",
        loanFromRejected.status === 400,
        `loan creation from rejected app → ${loanFromRejected.status} (${loanFromRejected.payload?.error?.message ?? ""})`,
      );
      const dbB1 = await dbRows(`SELECT status FROM loan_applications WHERE application_id = $1`, [
        ctx.applications.appB1.applicationId,
      ]);
      check(
        "app_b1_status_in_db",
        dbB1[0]?.status === "rejected",
        `rejected state persisted in PostgreSQL (${dbB1[0]?.status})`,
      );
    });
    ...
    await runStage("Loan creation (positive & cross-tenant guards)", async () => {
      ...
      const nonParticipant = await api("/loans", {
        method: "POST",
        token: lb.token,
        body: { applicationId: appAId },
      });
      check(
        "rule_b_non_participant_loan_creation",
        nonParticipant.status === 403,
        `lenderB loan creation on appA → ${nonParticipant.status}`,
      );

      const loan = await getLoan(la1.token, ctx.loanId);
      console.log(`  ✅ Loan created: ${loan.loanId} (principal ${loan.principalAmount} BDT)`);

      const loanRows = await dbRows(
        `SELECT loan_id, status, principal_amount FROM loans WHERE loan_id = $1`,
        [ctx.loanId],
      );
      check(
        "loan_in_db",
        loanRows.length > 0 && Number(loanRows[0].principal_amount) === 180000,
        `loan persisted in PostgreSQL (status=${loanRows[0]?.status})`,
      );

      // Full funding automatically creates and disburses the loan.
      const appStatusAfter = await dbRows(
        `SELECT status FROM loan_applications WHERE application_id = $1`,
        [appAId],
      );
      check(
        "rule_g_app_disbursed",
        appStatusAfter[0]?.status === "disbursed",
        `application state after full funding = '${appStatusAfter[0]?.status}' (disbursed)`,
      );
      const disbCount = await dbCount(
        `SELECT COUNT(*)::int AS count FROM loan_disbursements WHERE loan_id = $1`,
        [ctx.loanId],
      );
      check(
        "rule_g_disbursement_row_created",
        disbCount === 1,
        `loan_disbursements rows after full funding: ${disbCount}`,
      );

      // Rule E: duplicate loan creation rejected.
      const dupLoan = await api("/loans", {
        method: "POST",
        token: la2.token,
        body: { applicationId: appAId },
      });
      ...
      check(
        "borrower_sees_loan",
        baLoans.loans?.some((l: any) => l.loanId === ctx.loanId),
        "borrowerA sees loan via GET /loans",
      );
      ...
      check(
        "rule_c_borrower_app_isolation",
        crossApp.status === 403,
        `borrowerB reading appA → ${crossApp.status}`,
      );
      const crossLoan = await api(`/loans/${ctx.loanId}`, { token: bb.token });
      check(
        "rule_c_borrower_loan_isolation",
        crossLoan.status === 403,
        `borrowerB reading loanA → ${crossLoan.status}`,
      );
      ...
      check(
        "rule_b_lender_loan_isolation",
        crossLoanLb.status === 403,
        `lenderB reading loanA → ${crossLoanLb.status}`,
      );
      const crossTx = await api(`/loans/${ctx.loanId}/transactions`, { token: lb.token });
      check(
        "rule_b_lender_transactions_isolation",
        crossTx.status === 403,
        `lenderB reading loanA transactions → ${crossTx.status}`,
      );
      const lbLoans = await getLoans(lb.token);
      check(
        "rule_b_lender_list_isolation",
        !lbLoans.loans?.some((l: any) => l.loanId === ctx.loanId),
        "loanA absent from lenderB GET /loans",
      );
      const lbPortfolio = await getPortfolio(lb.token);
      check(
        "rule_b_lender_portfolio_isolation",
        !lbPortfolio.fundedLoans?.some((f: any) => f.applicationId === appAId),
        "loanA funding absent from lenderB portfolio",
      );

      // Lender sees the resulting loan through the lender API (workflow step 15).
      const la1Loan = await getLoan(la1.token, ctx.loanId);
      check(
        "lender_sees_loan_detail",
        la1Loan.loanId === ctx.loanId,
        "lenderA1 sees loan via GET /loans/:id",
      );
      ...
      check(
        "lender_sees_loan_in_portfolio",
        !!la1Funded,
        `lenderA1 portfolio contains loanA commitment (${la1Funded?.fundedAmount ?? 0} BDT)`,
      );
      const la1Tx = await getLoanTransactions(la1.token, ctx.loanId);
      check(
        "lender_sees_loan_transactions",
        Array.isArray(la1Tx),
        "lenderA1 can read loanA transactions",
      );
    });
    await runStage("Loan disbursement (real money transfer)", async () => {
      ...
      check(
        ...
        !!ctx.disbursementId,
        `disbursement recorded (${ctx.disbursementId})`,
      );

      const disbRows = await dbRows(
        `SELECT disbursement_id, amount FROM loan_disbursements WHERE loan_id = $1`,
        [ctx.loanId],
      );
      check(
        "disbursement_in_db",
        disbRows.length === 1 && Number(disbRows[0].amount) === 180000,
        `loan_disbursements rowin PostgreSQL: ${disbRows.length} (${disbRows[0]?.amount} BDT)`,
      );

      const appStatus = await dbRows(
        `SELECT status FROM loan_applications WHERE application_id = $1`,
        [ctx.applications.appA.applicationId],
      );
      check(
        "disbursement_moved_app_to_disbursed",
        appStatus[0]?.status === "disbursed",
        `application state after disbursement = '${appStatus[0]?.status}'`,
      );
      const viaApi = await getApplication(
        ctx.borrowers.ba.token,
        ctx.applications.appA.applicationId,
      );
      check(
        "disbursement_api_state",
        viaApi.status === "disbursed",
    ...
    await runStage("Repayment schedule generation", async () => {
      ...
      check(
        "schedules_generated",
        schedCount > 0,
        `${schedCount} repayment schedules generated for loan`,
      );
      const dbSched = await dbCount(
      ...
      check(
        "schedules_in_db",
        dbSched === schedCount,
        `repayment_schedules in PostgreSQL: ${dbSched}`,
      );

      const baSchedules = await getRepaymentSchedules(ctx.borrowers.ba.token, ctx.loanId);
      ctx.firstSchedule =
        baSchedules.schedules?.find((s: any) => Number(s.outstandingAmount) > 0) ?? null;
      check(
        "borrower_sees_schedules",
        !!ctx.firstSchedule,
        ctx.firstSchedule
          ? `next schedule ${ctx.firstSchedule.scheduleId} (${ctx.firstSchedule.expectedAmount} BDT)`
          : "no outstanding schedule",
      );
    });
    ...
    await runStage("Repayment via real API & state propagation", async () => {
      ...
      check(
        "schedule_marked_paid",
        payRows[0]?.schedule_status === "paid",
        `repayment_schedules.status = '${payRows[0]?.schedule_status}'`,
      );
      ...
      check(
        "loan_total_paid_updated",
        totalPaid > 0,
        `loan repaymentSummary.totalPaid = ${totalPaid}`,
      );
      ...
      check(
        "trust_score_repayment_event",
        Number(tsRows[0]?.is_current) === 1 && tsRows[0]?.trigger_event === "repayment_received",
        `latest trust score trigger = ${tsRows[0]?.trigger_event} (band ${tsRows[0]?.trust_band})`,
      );

      const la1Portfolio = await getPortfolio(ctx.lenders.la1.token);
      const entry = la1Portfolio.fundedLoans?.find(
        (f: any) => f.applicationId === ctx.applications.appA.applicationId,
      );
      const lenderTotalPaid = Number(entry?.totalPaid ?? 0);
      check(
        "lender_portfolio_reflects_repayment",
        lenderTotalPaid > 0,
        `lender portfolio totalPaid = ${lenderTotalPaid}`,
      );
      const tx = await getLoanTransactions(ctx.lenders.la1.token, ctx.loanId);
      check(
        "lender_transactions_show_repayment",
        tx.some((t: any) => t.type === "repayment"),
        "loan transactions include the repayment",
      );
    });
  } finally {
    printReport(ctx);
...
function printReport(ctx: WorkflowContext) {
  ...
  for (const k of Object.keys(ctx.lenders))
    console.log(`  - Lender ${k}:     ${ctx.lenders[k].username} (${ctx.lenders[k].userId})`);
  for (const k of Object.keys(ctx.borrowers))
    console.log(`  - Borrower ${k}:    ${ctx.borrowers[k].username} (${ctx.borrowers[k].userId})`);
  for (const k of Object.keys(ctx.applications))
    console.log(
      `  - Application ${k}: ${ctx.applications[k].applicationId} (${ctx.applications[k].purpose}, ${ctx.applications[k].amount} BDT)`,
    );
  ...
  for (const label of Object.keys(report)) {
    ...
    console.log(
      `  ${String(i++).padStart(2, "0")}. ${r.ok ? "PASS" : "FAIL"}  ${label} — ${r.detail}`,
    );
  }
