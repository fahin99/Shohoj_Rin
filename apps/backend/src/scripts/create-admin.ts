...
async function main() {
  ...
  if (existing_user.rowCount && existing_user.rowCount > 0) {
    ...
    if (user.role !== "admin")
      throw new Error(
        `User with username ${admin_user} or email ${admin_email} already exists but is not an admin`,
      );
  ...
  console.log(
    `Admin credentials: username=${admin_user}, email=${admin_email}, userid=${admin.user_id}`,
  );
  await closePool();
}
