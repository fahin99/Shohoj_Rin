...
router.post("/", requireAuth, async (req: RequestWithAuth, res) => {
  ...
  try {
    ...
    const insertedResult = await client.query(
      `SELECT schedule_id AS "scheduleId", loan_id AS "loanId", installment_number AS "installmentNumber",
              due_date AS "dueDate", expected_amount AS "expectedAmount", status
       FROM repayment_schedules
       WHERE loan_id = $1
       ORDER BY installment_number ASC`,
      [parsed.data.loanId],
    );
    const inserted = insertedResult.rows.map((row) => ({
      ...row,
    ...
    await client.query(
      ...
      [req.auth!.userId, parsed.data.loanId, parsed.data.loanId, inserted.length],
    );
