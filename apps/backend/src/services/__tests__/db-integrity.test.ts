...
describe('PostgreSQL Integrity Layer', () => {
  ...
  it('rejects negative requested_amount in loan_applications', async () => {
    ...
    const userIdRes = await pool.query(`INSERT INTO users (email, password_hash) VALUES ($1, 'hash') RETURNING user_id`, [email]);
  ...
  it('prevents updates to audit_logs due to append-only trigger', async () => {
    const logRes = await pool.query(`INSERT INTO audit_logs (action) VALUES ('test_action') RETURNING log_id`);
    const logId = logRes.rows[0].log_id;
    
    await expect(pool.query(
      `UPDATE audit_logs SET action = 'modified_action' WHERE log_id = $1`,
      [logId]
